...
function statusClass(status: string): string {
  ...
  if (s === 'active' || s === 'confirmed' || s === 'planned') return 'ck-fc-ev--active';
  if (s === 'done' || s === 'completed') return 'ck-fc-ev--done';
  if (s === 'cancelled') return 'ck-fc-ev--cancelled';
  return 'ck-fc-ev--neutral';
}
...
export function classSessionToEvents(rows: ClassSessionLike[]): ScheduleFcEvent[] {
  ...
  for (const row of rows) {
    ...
    out.push({
      ...
      url: href,
      display: 'auto',
      classNames: ['ck-fc-ev', 'ck-fc-ev--timed', statusClass(row.status)],
      extendedProps: {
...
export function classBatchToEvents(rows: ClassBatchLike[]): ScheduleFcEvent[] {
  ...
  for (const row of rows) {
    ...
    out.push({
      ...
      url: href,
      display: 'block',
      classNames: ['ck-fc-ev', statusClass(row.status)],
      extendedProps: {
        batchId: row.id,
        program: row.program,
        status: row.status,
