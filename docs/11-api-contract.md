...
> **product-decision 2026-07-07**: Auth LMS đảo 2 tầng — ngược với QĐ0033/WF-P1-07 cũ (phone+OTP đơn tài khoản). Hành vi trước đây: phụ huynh đăng nhập bằng SĐT + OTP SMS/phone. Hành vi hiện tại: (a) **Phụ huynh** (`kind='parent'`) đăng nhập bằng **email + OTP qua email**; procedure: `lmsAuth.requestOtpEmail` / `lmsAuth.verifyOtpEmail`. (b) **Học sinh** (`kind='student'`) đăng nhập bằng **SĐT phụ huynh + password**; procedure: `lmsAuth.loginStudent`. `LmsSubject.kind` là discriminator phân tách session. Tham chiếu: UI implementation plan phase 01a/01b.
>
> Worker prod dùng `BrevoEmailTransport` (console cấm ở prod, chỉ dev/test). OTP PH **được enqueue** vào EmailOutbox; lỗi hiện tại trên prod là `BREVO_API_KEY` invalid (HTTP 401), không phải Console-only / "không gửi email thật".
