...
(file test viết & chạy xanh, đã tồn tại từ 2026-07-10 — 532 tests passing, 0 skipped, 64 test files, live run 2026-07-11) · `ADR/QĐ` · `Oversight`.
...
| **P3-01** | nhân viên | "Chấm công qua WiFi công ty" | `checkInOut.punch` (checkInOut.punch) | `/attendance/check-in-out` | `apps/api/src/checkin/ip-match.test.ts` | **ADR0039** | người |
| **P3-02** | nhân viên / manager | "Duyệt chấm công thủ công" | `manualPunch.create/approve` (manager) | `/attendance/check-in-out` | `apps/api/src/checkin/ip-match.test.ts` | **ADR0039** · QĐ0034 | HITL |
| **P3-03** | sale / giao_vien | "Đăng ký ca làm" | `shiftRegistration.submit` (shift.register) | `/attendance/shifts` | `apps/api/src/shift/register-approve.test.ts` | **ADR0040** · QĐ0035 | HITL |
| **P3-04** | manager / GĐ | "Duyệt ca" | `shiftRegistration.approve` (managerId/GĐ) | `/attendance/shifts/:id` | `apps/api/src/shift/register-approve.test.ts` | **ADR0040** · QĐ0027 | HITL |
| **P3-05** | GĐ | "Chốt lương tháng" | `payroll.finalize` · `compensation.upsertRate` | `/hr/payroll/:id` · `/hr/salary-structure` | `apps/api/src/payroll/penalty-posttax.test.ts` | QĐ0025/0012 | HITL |
| **P3-06** | hệ thống / GĐ | "Chấm & duyệt KPI" | `kpi.score/confirm/approve` | `/hr/kpi` | `apps/api/src/kpi/override-tree.test.ts` | QĐ0011/0010 | HITL |
