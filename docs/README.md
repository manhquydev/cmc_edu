...
| **0038** | Mở bài tập theo lần phát — bài mở khi đã phát `SessionExercise` trên buổi chưa hủy **và** HS `onRoster` (dải unit bắt buộc). *(2026-08-12: **Tier A/B** + hai cờ `LMS_OPEN_TIER_ENABLED` / `LMS_ENTITLEMENT_GATE` đã gỡ — PR #118; bài nộp khóa `(sessionExerciseId, studentId)`)* |
