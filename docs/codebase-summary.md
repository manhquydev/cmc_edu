...
>
> **Updated 2026-07-26 (nghiệm thu journey + CI khôi phục):** e2e hiện có **43 spec file** — gồm **31 journey UI** (`apps/e2e/tests/journeys/*.journey.ui.spec.ts`), tất cả chạy trong project `ui-chromium` trên CI. Sổ nghiệm thu: **31/38 luồng đã chứng minh chạy** (commit `324bd12`), cấu trúc 37 built / 1 partial / 0 missing, actor-audit 0 phát hiện. **Đây là TRẦN của phương pháp journey**: 7 luồng còn lại là `no-ui-path` — không có UI nên journey không thể lái, muốn phủ phải xây UI trước (thuộc plan sửa, không thuộc plan đo). ⇒ **38/38 luồng đều có trạng thái máy-chứng, 0 luồng chưa phân loại.** Đây là bằng chứng đầu tiên do CI sinh (`gitDirty:false`) — trước 2026-07-26 chỉ có run cục bộ mang nhãn tham khảo. **Giới hạn:** journey ở mức smoke — chứng minh luồng *chạy thông*, KHÔNG chứng minh *đúng số học nghiệp vụ*; UAT người thật (M0) vẫn chưa chạy. Test đơn vị/tích hợp đo trên CI cùng commit: `@cmc/api` 104 file/988 test, `@cmc/admin` 39 file/396 test, `@cmc/ui` 12/45, `@cmc/domain-payroll` 2/38, `@cmc/domain-time` 1/31, `@cmc/domain-finance` 5/17.
...
> *(Số e2e "20 pass" đo 2026-07-23; sau đó thêm journey — tới 2026-07-26 có **31 journey UI / 43 spec file**, CI chạy `ui-chromium` xanh tại commit `324bd12`. Số api/admin cập nhật cùng ngày: api **988**, admin **396**.)*
>
...
> Tầng mạnh hơn là **journey UI thật chạy xanh**: hiện **31/38 luồng** tại commit `324bd12`, sinh từ artifact
> CI `ui-e2e` (`gitDirty:false`) — trước 2026-07-26 chỉ có run cục bộ mang nhãn tham khảo. Trần khả thi qua
> journey = **31/38** — đã chạm trần; 7 luồng còn lại đều `no-ui-path` có hồ sơ.
>
