...
Ngoài các milestone dọc tuyến tính M0→M4, **UI migration spike đã hoàn thành toàn bộ (Phases 1–5)**:
...
- **Phase 5 (complete, 2026-07-10):** gỡ hoàn toàn `@mantine/*` deps (0 match trong package.json / source / pnpm-lock), sync docs design-system + tech-stack sang Astryx. PR #28 CI-green. AC#1 = "Zero Mantine". Xem `docs/journals/260710-astryx-phase5-completion.md`.
...
- **Plan:** `plans/260710-0236-astryx-ui-migration/` (5 pha, tracked development context).
...
|---|---|---|---|---|---|
| **M0** | Go-live sprint | Land SSO (PR #24) → ENV `cmcv2-prod` local-sim → UAT 2-run + người thật | Biên bản GO ký; e2e critical 2/2 PASS prod-config; email live Brevo+Graph; tracker #8/#9/#10 completed | `plans/260707-2308-golive-sprint-land-sso-env-uat` | **Đang chạy** — SSO đã land; sổ nghiệm thu **31/38 luồng chứng minh chạy** (CI 2026-07-26, commit `324bd12`, đã chạm trần journey); **UAT người thật CHƯA chạy** ⇒ M0 chưa đóng |
...
| **M2** | P4 completion | Lịch test (WF-P4-04) · after-sale case (WF-P4-05) · họp PH audit đầy-cuối · **lịch họp PH theo LỚP + nhắc + GV xem để chuẩn bị** (PO chốt 2026-07-23 hoãn sau go-live: `ParentMeeting` hiện gắn từng học sinh, không có `classBatchId`; cột nhắc đã bị bỏ và có test khoá; cấp quyền đọc cho GV lúc này sẽ lộ tên mọi trẻ trong cơ sở vì không có gì để lọc theo lớp — cần sửa schema + khôi phục nhắc + scope bằng `assert-teacher-owns-class.ts`) · ✅ test coverage P4-adjacent (CLOSED 2026-07-10 via commit 326dfcc) | Acceptance TL28 pass; trace matrix P4 không ô trống; gates xanh; TL25 P4 test refs verified | tạo khi M1 gần xong | Chưa — P4 test-gap sub-item DONE (2026-07-10); other sub-items remain open |
