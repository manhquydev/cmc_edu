...
describe('MetricCard', () => {
  it('renders the value; a status shows a DOT and never recolours the value', () => {
    ...
    const value = container.querySelector('.ck-mc-value')!;
    ...
    expect(container.querySelector('.ck-attn')).toBeInTheDocument();
  });

  it('renders no attention dot when omitted', () => {
    const { container } = renderCard(base);
    expect(container.querySelector('.ck-attn')).toBeNull();
  });

  it('renders a monochrome outline icon (currentColor, no fill)', () => {
    const { container } = renderCard(base);
    const svg = container.querySelector('.ck-mc-icon svg')!;
  ...
  it('loading renders a skeleton instead of the value', () => {
    ...
    expect(container.querySelector('.ck-mc-value')).toBeNull();
  });
