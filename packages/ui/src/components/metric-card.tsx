...
// Requires @cmc/ui/console.css (.console-mc* classes).
...
export function MetricCard({ label, value, context, icon, href, attention, loading }: MetricCardProps) {
  return (
    <Link to={href} className="console-mc">
      <div className="console-mc-top">
        <span className="console-mc-label">
          {attention && <span className="console-attn" style={{ background: toneColor(attention) }} />}
          {label}
        </span>
        <span className="console-mc-icon"><LineIcon name={icon} size={19} /></span>
      </div>
      {loading
        ? <Skeleton height={32} width="46%" radius={0} />
        : <div className="console-mc-value">{value}</div>}
      <div className="console-mc-ctx">{context}<LineIcon name="chevron" size={13} /></div>
    </Link>
  );
}
