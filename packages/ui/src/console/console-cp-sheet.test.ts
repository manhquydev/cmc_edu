...
describe('odoo.css ControlBar densify + form sheet', () => {
  ...
  it('flattens filter-bar inside control-bar under shell', () => {
    const body = ruleBlock('.o_web_client .console-control-bar .console-filter-bar');
  ...
  it('ships form sheet dual-layer base classes', () => {
    expect(ruleBlock('.console-form-sheet-bg')).toBeTruthy();
    expect(ruleBlock('.console-form-sheet')).toBeTruthy();
    const sheet = ruleBlock('.console-form-sheet');
